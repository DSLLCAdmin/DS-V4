<!-- Updated: 2025-08-30T20:54:08.129Z -->
const fs = require('fs');
...
class SmartMerger {
  constructor() {
    ...
    this.backupPath = path.join(__dirname, '../data/products-backup.ts');
  ...
  backupCurrent() {
    if (fs.existsSync(this.productsPath)) {
      ...
      console.log('✅ Current products.ts backed up to products-backup.ts');
    }
  }
  ...
  smartMerge(currentProducts, excelProducts) {
    ...
    for (const excelProduct of excelProducts) {
      ...
      if (currentProduct) {
        ...
        // Update NON-critical fields from Excel
        for (const [key, value] of Object.entries(excelProduct)) {
          // NEVER overwrite these critical fields
          if (key === 'image' || key === 'Type') {
            continue;
          }
          
          // Update Title only if it's not empty and not a category header
          if (key === 'Title' && value && value.trim() !== "" && value.length > 2 && !value.endsWith(':')) {
