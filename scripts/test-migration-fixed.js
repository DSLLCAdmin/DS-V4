...
// Simulate Shopify products with numeric IDs for URL compatibility
const mockShopifyProducts = [
  { id: '1234567890', title: 'First & Light- E-book', handle: 'first-light-ebook' },
  { id: '1234567891', title: 'First & Light- Paperback', handle: 'first-light-paperback' },
  { id: '1234567892', title: 'Risque & Safety- E-book', handle: 'risque-safety-ebook' },
  { id: '1234567893', title: 'Risque & Safety- Paperback', handle: 'risque-safety-paperback' },
  { id: '1234567894', title: 'Mercury & Memory- E-book', handle: 'mercury-memory-ebook' },
  { id: '1234567895', title: 'Mercury & Memory- Paperback', handle: 'mercury-memory-paperback' },
  { id: '1234567896', title: 'DarkStreet Tees', handle: 'darkstreet-tees' },
  { id: '1234567897', title: 'Hats', handle: 'hats' },
  { id: '1234567898', title: 'DarkStreet Mugs', handle: 'darkstreet-mugs' },
  { id: '1234567899', title: 'Official DarkStreet Driving Playlists', handle: 'darkstreet-playlists' },
  { id: '1234567900', title: 'DS Route Generator App', handle: 'ds-route-generator' }
];
...
function migrateProductsWithCategories(shopifyProducts) {
  console.log('\n📝 FIXED MIGRATION: Using numeric Shopify IDs for URL compatibility...\n');
...
function testPlatformFunctionality() {
  ...
  try {
    ...
    if (productsMatch) {
      ...
      const shopifyIds = products.filter(p => /^\d{10}$/.test(p.id));
      ...
      if (alphaIds.length > 0) {
        console.log('   Remaining IDs:', alphaIds.slice(0, 10).map(p => `${p.id} (${p.title})`).join(', '));
        if (alphaIds.length > 10) {
          console.log(`   ... and ${alphaIds.length - 10} more`);
        }
      }
      ...
      if (urlIncompatible.length === 0) {
        ...
        urlIncompatible.forEach(p => console.log(`   - ${p.id} (${p.title})`));
      }
      
      return { success: true, products, shopifyIds, alphaIds, urlIncompatible };
    }
  } catch (error) {
...
async function main() {
  console.log('🔄 FIXED: Shopify ID Migration Test (Numeric IDs)\n');
  ...
  if (testResults.success && testResults.urlIncompatible.length === 0) {
    ...
    console.log('   node scripts/test-migration-fixed.js --restore');
  } else {
